...
export default function LibraryPage() {
  ...
  /**
   * ✅ (1) helper demandé : reload + scroll top + refresh immédiat
   */
  const onJobCreatedNow = async () => {
    // 1) afficher le nouveau job immédiatement
    await loadJobsSilent();
    ...
    // 3) refresh immédiat (sans attendre l’interval)
    await refreshAll({ force: true });
  };

  /**
   * ✅ Bonus : refresh immédiat au mount
   */
  useEffect(() => {
...
function JobCard({
  ...
  const submitRemix = async () => {
    ...
    try {
      ...
      // ✅ (2) IMPORTANT : afficher le job remix tout de suite + scroll top + refresh immédiat
      await onJobCreated();
    ...
    >
      ...
      <div style={{ padding: 12 }}>
        ...
        {showRemix && (
          ...
          >
            <div style={{ fontWeight: 900, fontSize: 13 }}>Remix (1 crédit, débité à la réussite)</div>
